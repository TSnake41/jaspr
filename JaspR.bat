...
::INSCRIPTION DE LA TAILLE TOTALE DANS LE RAPPORT
	if not %ttlclnsz% gtr 0 (
		if not %analyse% equ 1 (
			echo Aucune donnée n'a été effacée ! >>"%tmp%\rapport.tmp"
		) else (
			echo Aucune donnée à supprimer ! >>"%tmp%\rapport.tmp"
		)
	) else (
		echo.>>"%tmp%\rapport.tmp"
			if not %analyse% equ 1 (
				echo Environ %ttlclnsz% Mo supprimé^(s^). >>"%tmp%\rapport.tmp"
			) else (
				echo Environ %ttlclnsz% Mo à supprimer. >>"%tmp%\rapport.tmp"
			)
	)
...
set /a cl_recent_size=(%ttlclnsz%-%cl_temp_size%*1024*1024-%cl_winlog_size%*1024*1024-%cl_thumb_size%*1024*1024-%cl_trash_size%*1024*1024)/1024/1024
	if %cl_recent_size% gtr 0 (
		echo Liste des Fichiers Récents : %cl_recent_size% Mo >>"%tmp%\rapport.tmp"
	)
if %yes_clean_nav% equ 1 (
cls
echo.
...
set /a cl_prefetch_size=(%ttlclnsz%-%cl_temp_size%*1024*1024-%cl_winlog_size%*1024*1024-%cl_thumb_size%*1024*1024-%cl_trash_size%*1024*1024-%cl_recent_size%*1024*1024)/1024/1024
	if %cl_prefetch_size% gtr 0 (
		echo Vieilles Données du Prefetch : %cl_prefetch_size% Mo >>"%tmp%\rapport.tmp"
	)
if %yes_clean_nav% equ 1 (
cls
echo.
...
set /a hiberfilsizemo=%hiberfilsize%/1024/1024
	if defined hiberfilsize (
		if %hiberfilsize% gtr 0 echo Supp. Mise en Veille Prolongée : %hiberfilsizemo% Mo >>"%tmp%\rapport.tmp"
	)
if %yes_clean_nav% equ 1 (
cls
echo.
...
::checkbox "recentfiles" "Liste des Fichiers RÃ©cents" TRUE 20 130 180 25
...
::checkbox "prefetch" "Vieilles donnÃ©es du prefetch" TRUE 200 130 180 25
::checkbox "hiberstate" "DÃ©sactiver la mise en veille prolongÃ©e" FALSE 20 160 250 25
